...
const CACHE_DIR = process.argv[3] || os.tmpdir();
...
function generateTypescript(images) {
    ...
    const imageImports = images.map((image) => {
        const pascalName = toPascalCase(image.name);
        return dedent`
            import ${pascalName}Avif from "./images/${image.name}.avif";
            import ${pascalName}Webp from "./images/${image.name}.webp";
            import ${pascalName}Jpg from "./images/${image.name}.jpg";
    ...
    const content = [preamble, ...imageImports].join("\n\n");
...
(async () => {
    ...
    const tsContent = generateTypescript(IMAGES);
