            ...
            {resolvedResults.length > 0 && (
                resolvedResults.map((res, i) => (
                    <div key={res.uri}>
                        ...
                        <Pre data-copy="" className="-mt-5"><Code>{res.uri}</Code></Pre>
                        {'result' in res && (
                            <>
                                <span className="text-sm text-gray-500">Resolved</span>
                                <Pre data-copy="" className="-mt-5"><Code>{res.result.url}</Code></Pre>
                                {res.result.lastModified && (
                                    <>
                                        <span className="text-sm text-gray-500 block">Last Modified</span>
                                        <Pre data-copy="" className="-mt-5"><Code>{dayjs().to(res.result.lastModified)} ({res.result.lastModified.toISOString()})</Code></Pre>
                                    </>

                                )}
                                <>
                                    <span className="text-sm text-gray-500 block">Expires</span>
                                    <Pre data-copy="" className="-mt-5"><Code>{
                                ...
                                {res.result.headers && (
                                    <>
                                        ...
                                        <Pre data-copy="" className="-mt-5"><Code>{JSON.stringify(Object.fromEntries(res.result.headers), null, 2)}</Code></Pre>
                                    </>
                                )}
                            </>
                        )}
    ...
    return (
        ...
        {successfulUploads.map(({name, uri}) => (
            <div key={uri}>
                ...
                <Pre data-copy="" className="-mt-5"><Code>{uri}</Code></Pre>
            </div>
        ))}
